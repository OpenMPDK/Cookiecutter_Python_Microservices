...
def validate_url_prefix(value):
    """
    validates the give url prefix and returns back the updated url prefix
    """
    if value:
        if not value.startswith('/'):
            raise ArgumentError('URL Prefixess must start with a /')
...
class RestServer(Command):
    ...
    def get_parser(self, program_name):
        ...
        server_config_subparser = subparser.add_parser('setnwcfg',help='Managing the Rest server network configuration')
        ...
        server_config_subparser = subparser.add_parser('seturprefix',help='Managing the Rest server URL Prefix')
        server_config_subparser.add_argument(
            '-p',
            '--urlprefix',
            action='store',
    ...
    def take_action(self, parsed_arguments):
        """
        Take command based action on the argument passed to this module
        """
        ...
        if parsed_arguments.rest_server_command == 'setnwcfg':
            self.app.set_rest_server_network_config(parsed_arguments.rest_server_host, parsed_arguments.rest_server_port)
            ...
            self.LOG.info('Restar the rest server to reflect this change')
        ...
        if parsed_arguments.rest_server_command == 'service':
            ...
            if parsed_arguments.deregister_rest_server_as_service:
                ...
                self.LOG.info('Deregistered rest server as a service')