import threading
from Utils.Exception.SingletonException import SingletonException
    ...
    try:
        ...
        try:
            ...
            if str(e).find('__init__() takes') != -1:
                raise SingletonException('Pass __init__ args on first call to get_instance()')
            else:
                raise
...
def release_instances():
    ...
    try:
        for cls in _singletons.copy():
            cls.release_class_instances_for_testing()
            ...
            if len_singletons > 0:
                for _cls in _singletons.copy():
                    _cls.release_class_instances_for_testing()
                    len_singletons -= 1
...
class SingletonMeta(type):
    ...
    def __call__(cls, *args, **kwargs):
        raise SingletonException('Singletons may be instantiated only through getInstances()')
...
class Singleton(object, metaclass=SingletonMeta):
    ...
    def get_instance(cls, *args, **kwargs):
        """
        Gets the already created instance of a class object or creates a new instance
        Args:
            *args:
            **kwargs:
    ...
    @classmethod
    def release_class_instances_for_testing(cls):
        """
        ...
        try:
            if hasattr(cls.cInstance, '_prepare_to_forget_singleton'):
                cls.cInstance.prepare_to_forget_singleton()
            del cls.cInstance
            _remove(cls)
        except AttributeError:
            for baseClass in cls.__bases__:
                if issubclass(baseClass, Singleton):
                    baseClass.release_class_instances_for_testing()
